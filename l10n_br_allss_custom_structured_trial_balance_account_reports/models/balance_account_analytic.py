    ...
    def execute_sql(self):
        ...
        cr.execute("""
            ...
            SELECT
                ...
                COALESCE(
                    LAG(f.final_balance) OVER (
                        PARTITION BY f.company_id, f.analytic_account_id
                        ORDER BY f.date
                    ),
                    0
                ) AS allss_previous_balance,
            ...
            =============================== */
            JOIN account_analytic_account a0 ON a0.id = f.analytic_account_id
            LEFT JOIN account_analytic_account a1 ON a1.id = a0.parent_id
            LEFT JOIN account_analytic_account a2 ON a2.id = a1.parent_id
            LEFT JOIN account_analytic_account a3 ON a3.id = a2.parent_id
            LEFT JOIN account_analytic_account a4 ON a4.id = a3.parent_id
        ...
        cr.execute("""
            SELECT setval(
                'allss_balance_account_analytic_id_seq',
                COALESCE((SELECT MAX(id) FROM allss_balance_account_analytic), 1),
                true
            )
        """)
