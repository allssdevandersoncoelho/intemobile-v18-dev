...
class AllssAccountMoveNfeImport(models.Model):
    ...
    def get_ide(self, nfe, operacao, fiscal_position_id):
        ...
        return dict(
            ...
            l10n_latam_document_type_id=serie.sudo().l10n_br_allss_latam_document_type_id.id,
        )
        ...
        if hasattr(nfe.NFe.infNFe, 'pag'):
            ...
            if len(pag) and hasattr(pag, 'detPag'):
                if hasattr(pag.detPag, 'card'):
                    ...
                    return {'l10n_br_allss_payment_mode_id': cnpj_cpf and self.env.get(
    ...
    def import_nfe(self, auto, company_id, nfe, nfe_xml, dfe, 
        ...
        _logger.warning(f">>>>> ALLSS > import_nfe > account_move_dict 🟢 type(nfe.NFe.pag) {type(nfe.NFe.pag)} -- type(nfe.NFe.pag[0]){type(nfe.NFe.pag[0])}")
    ...
    def _create_product(self, partner_id, nfe_item, uom_id=False):
        ...
        seq_id = int(params.get_param(
            'l10n_br_allss_nfe_import.l10n_br_allss_product_sequence_id', default=False))
