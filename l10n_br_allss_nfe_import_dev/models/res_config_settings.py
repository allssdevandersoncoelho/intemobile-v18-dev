...
class L10nBrAllssResConfigSettings(models.TransientModel):
    ...
    l10n_br_allss_product_sequence_id = fields.Many2one(
        ...
        default=lambda self: (
            self.env.ref(
                "l10n_br_allss_nfe_import.sequence_l10n_br_allss_nfe_import_product",
                raise_if_not_found=False,
            )
            or self.env['ir.sequence'].search(
                [('code', '=', 'l10n_br_allss_nfe_import.product')], limit=1
            )
            or False
        ),
    )
    ...
    def get_values(self):
        ...
        res.update(
            l10n_br_allss_product_sequence_id=int(params.get_param(
                'l10n_br_allss_nfe_import.l10n_br_allss_product_sequence_id', default=0)),
            l10n_br_allss_ms_code_for_nfe_import=int(params.get_param(
                'l10n_br_allss_nfe_import.l10n_br_allss_ms_code_for_nfe_import', default=0))
            or self.env.get('l10n.br.allss.ms.line'),
            l10n_br_allss_nfe_import_nfe_mde_limit_for_process=int(params.get_param(
                'l10n_br_allss_nfe_import.l10n_br_allss_nfe_import_nfe_mde_limit_for_process', default=50)),
        )
        return res
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            'l10n_br_allss_nfe_import.l10n_br_allss_product_sequence_id',
            self.l10n_br_allss_product_sequence_id.id)
        self.env['ir.config_parameter'].sudo().set_param(
            'l10n_br_allss_nfe_import.l10n_br_allss_ms_code_for_nfe_import',
            self.l10n_br_allss_ms_code_for_nfe_import.id)
        self.env['ir.config_parameter'].sudo().set_param(
            'l10n_br_allss_nfe_import.l10n_br_allss_nfe_import_nfe_mde_limit_for_process',
